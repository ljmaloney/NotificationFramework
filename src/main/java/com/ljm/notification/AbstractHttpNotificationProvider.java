...
public abstract class AbstractHttpNotificationProvider extends AbstractNotificationProvider
{
    ...
    public void initialize(HashMap configMap)
    {
        ...
        LogManager.getLogger(getClass().getName()).info("[initialize()] - Provider initialized with [httpUrl="+httpUrl+"]");
    }
    ...
    */
    ...
    {	
    	LogManager.getLogger(getClass().getName()).debug("Send notification GET request "+httpUrl);
    	LogManager.getLogger(getClass().getName()).debug("[sendGetRequest()] - query string is "+queryString);
        ...
        if ( LogManager.getLogger(getClass().getName()).isDebugEnabled())
        	LogManager.getLogger(getClass().getName()).debug("Elapsed time to send GET request to "+httpUrl+" is "+(stopTime-startTime)+" milliseconds");
    ...
    protected XanbooHttpResponse sendPostRequest(String queryString)throws MalformedURLException, IOException, Exception
    {
        if ( LogManager.getLogger(getClass().getName()).isDebugEnabled())
        {
        	LogManager.getLogger(getClass().getName()).debug("Send notification POST request "+httpUrl);	
        	LogManager.getLogger(getClass().getName()).debug("[sendPostRequest()] - query string is "+queryString);
        }
        XanbooHttpConnector xanbooHttpConnector = new XanbooHttpConnector();
        XanbooHttpResponse resp = null;
        ...
        if ( LogManager.getLogger(getClass().getName()).isDebugEnabled())
        	LogManager.getLogger(getClass().getName()).debug("Elapsed time to send POST request to "+httpUrl+" is "+(stopTime-startTime)+" milliseconds");
