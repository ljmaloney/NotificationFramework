 ...
 * @since October 2013
 */
public abstract class AbstractNotificationProvider implements NotificationProviderInterface
{
    ...
    */
    ...
    public void initialize(HashMap configMap) 
    {
        ...
        LogManager.getLogger(getClass().getName()).info("[initialize()] Initializing notification provider [profileTypeId="+profileTypeId+"]");
    ...
    public void validate(NotificationMessageInterface message)
    {
        ...
        if ( maxLenStr != null && !maxLenStr.equalsIgnoreCase(""))
        {
            ...
            if (  !maxLen.equals(-1) && !maxLen.equals(0))
            {
                ...
                {
                	LogManager.getLogger(getClass().getName()).warn("[AbstractNotificationProvider.validate()] - Message length of "+message.getMessage().length()+" is more than the max length "+maxLen);
        ...
        else 
        {
            if ( LogManager.getLogger(getClass().getName()).isDebugEnabled())
            	LogManager.getLogger(getClass().getName()).debug("[validate()] maxlength not configured for provider");
        }
    }
    /**
    ...
    */
    ...
    {
        if ( !(provider2 instanceof NotificationProviderInterface) )
            return false;

        NotificationProviderInterface pi = (NotificationProviderInterface)provider2;
