...
public abstract class AbstractSMTPNotificationProvider extends AbstractNotificationProvider
{
    ...
    */
    ...
    public void initialize(HashMap config)
    {
        ...
        LoggerFactory.getLogger(getClass().getName()).info("[initialize()] - configuration properties "+config);
        ...
        if ( config.containsKey(EMAIL_JNDI))
        {
            ...
            LoggerFactory.getLogger(getClass().getName()).info("[initialize()] - Provider intialized with - [emailJNDIName="+emailJNDIName+"]");
        }
        else
        {
            ...
            LoggerFactory.getLogger(getClass().getName()).info("[initialize()] - Provider intialized with - [emailHostName="+emailHostName+",transportProtocol="+transportProtocol+",debugEmail="+debugEmail+"]");
        }
    ...
    */
    protected Session getMailSession()throws Exception
    {
        ...
        if ( LoggerFactory.getLogger(getClass().getName()).isDebugEnabled())
            LoggerFactory.getLogger(getClass().getName()).debug("[getMailSession()] - Got MailSession using mail.smtp.host="+emailHostName);
        return mailSession;
    }
    /**
    ...
    protected InternetAddress[] getInternetAddress(List<NotificationDestination> destinations)
    {
        ...
        for ( NotificationDestination dest : destinations )
        {
            ...
            catch(AddressException ae)
            {
                ...
                LoggerFactory.getLogger(getClass().getName()).info("[getInternetAddress()] - Invalid destination address - "+dest.getDestinationAddress(),ae);
            }
        }
        return addresses;
    }
    ...
    */
    protected void sendEmail(String subject,String content,InternetAddress[] toAddr,InternetAddress fromAddr)
                                throws  XanbooNotificationException,NoSuchProviderException,MessagingException,Exception
    {
    ...
    */
                            ...
                            throws XanbooNotificationException,NoSuchProviderException,MessagingException,Exception
    {
        ...
        {
            throw new XanbooNotificationException("Exception obtaining connection to email host "+super.getConfigMap(),ex);
        }
        ...
        else	//attachments have been added to the message ... 
        {
            ...
            for ( javax.activation.DataSource ds : attachments )
            {
                ...
                if ( LoggerFactory.getLogger(getClass().getName()).isDebugEnabled())
                    LoggerFactory.getLogger(getClass().getName()).debug("[sendEmail] - Added attachment to email " +ds.getName());
            }
        ...
        if ( LoggerFactory.getLogger(getClass().getName()).isDebugEnabled())
            LoggerFactory.getLogger(getClass().getName()).debug("[sendEmail] - Elapsed time to send notification email : " + (stopTime-startTime)+" milliseconds");
    }
}
