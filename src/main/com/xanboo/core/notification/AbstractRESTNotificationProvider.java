...
public abstract class AbstractRESTNotificationProvider extends AbstractHttpNotificationProvider
{
	...
	public void initialize(HashMap config)
	{
		...
		LoggerFactory.getLogger(getClass().getName()).info("[initialize()] Initialize REST Provider");
    ...
    {
        LoggerFactory.getLogger(getClass().getName()).debug("[sendPostRequest()] - send notification using "+this.getUrl()+" requestData="+requestData);
        ...
        if ( LoggerFactory.getLogger(getClass().getName()).isDebugEnabled())
            LoggerFactory.getLogger(getClass().getName()).debug("[sendPostRequest()] - http headers = "+headers.toString());
